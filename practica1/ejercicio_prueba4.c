#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "afnd.h"
#include "transforma.h"

int main(int argc, char ** argv) {

	AFND * p_afnd;
	AFND * afd;

  p_afnd = AFNDNuevo("afd_prueba4", 6, 3);

	/*
	Juntamos los símobolos del 0 al 9 para facilitar la comprensión visual del afd
	*/

	AFNDInsertaSimbolo(p_afnd,"0,...,9");
	AFNDInsertaSimbolo(p_afnd,"+");
	AFNDInsertaSimbolo(p_afnd,"-");

	AFNDInsertaEstado(p_afnd, "q0",INICIAL);
	AFNDInsertaEstado(p_afnd, "q1", NORMAL);
	AFNDInsertaEstado(p_afnd, "q2", NORMAL);
	AFNDInsertaEstado(p_afnd, "q3", NORMAL);
	AFNDInsertaEstado(p_afnd, "q4", NORMAL);
	AFNDInsertaEstado(p_afnd, "q5", FINAL);

	AFNDInsertaTransicion(p_afnd, "q0", "-", "q1");
    AFNDInsertaTransicion(p_afnd, "q1", "0,...,9", "q1");
    AFNDInsertaTransicion(p_afnd, "q1", "0,...,9", "q4");
    AFNDInsertaTransicion(p_afnd, "q1", "-", "q2");
    AFNDInsertaTransicion(p_afnd, "q2", "+", "q4");
    AFNDInsertaTransicion(p_afnd, "q1", "+", "q3");
    AFNDInsertaTransicion(p_afnd, "q3", "+", "q5");
    AFNDInsertaTransicion(p_afnd, "q3", "-", "q5");

	AFNDInsertaLTransicion(p_afnd, "q0", "q1");
    AFNDInsertaLTransicion(p_afnd, "q1", "q2");
    AFNDInsertaLTransicion(p_afnd, "q2", "q4");
    AFNDInsertaLTransicion(p_afnd, "q4", "q5");

	AFNDCierraLTransicion(p_afnd);

	afd  = AFNDTransforma(p_afnd);
	AFNDImprime(stdout,afd);
	AFNDADot(afd);
	AFNDADot(p_afnd);

	AFNDElimina(afd);
	AFNDElimina(p_afnd);

	return 0;
}